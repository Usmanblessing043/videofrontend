...
const Meetingroom = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      await ensureLocalStream(); // ✅ get camera/mic before signaling
      ...
      s.on("user-joined", async ({ socketId, username }) => {
        if (socketId === s.id) return; // ignore self
      ...
      s.on("signal", async ({ from, data }) => {
        ...
        if (data.type === "offer") {
          ...
          const answer = await pc.createAnswer();
          await pc.setLocalDescription(answer);
          s.emit("signal", { roomId, to: from, data: { type: "answer", sdp: answer } });
        } else if (data.type === "answer") {
          await pc.setRemoteDescription(data.sdp);
        } else if (data.type === "ice-candidate" && data.candidate) {
          try {
  ...
  const leaveMeeting = (manual = true) => {
    if (socketRef.current && roomId) socketRef.current.emit("leave-room", { roomId });
    stopAll();
    if (manual) navigate("/dashboard");
  };
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", padding: "12px 16px", borderBottom: "1px solid #eee", gap: 12 }}>
        ...
        <button onClick={toggleVideo}>{videoEnabled ? "Stop Video" : "Start Video"}</button>
        <button onClick={shareScreen}>Share Screen</button>
        <button onClick={endMeeting} style={{ color: "red" }}>End Meeting</button>
      </div>

      {/* Main content */}
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "3fr 1fr" }}>
        {/* Video Grid */}
        <div style={{ flex: 1, padding: 16, background: "#f8fafc", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 12 }}>
          <VideoTile streamRef={localVideoRef} label={userData.isCreator ? "You" : userData.email} videoEnabled={videoEnabled} />
          {Object.entries(remoteStreams).map(([peerId, stream]) => (
            <RemoteTile key={peerId} stream={stream} label={participants[peerId] || peerId.slice(0, 6)} />
          ))}
        </div>

        {/* Chat */}
        <div style={{ borderLeft: "1px solid #ddd", display: "flex", flexDirection: "column", background: "#fff" }}>
          <div style={{ flex: 1, overflowY: "auto", padding: 12, fontSize: 14, display: "flex", flexDirection: "column" }}>
            {messages.map((m, i) => {
              const isSelf = m.user === userData.email;
              return (
                <div key={i} style={{ alignSelf: isSelf ? "flex-end" : "flex-start", background: isSelf ? "#dcf8c6" : "#f1f0f0", padding: "8px 12px", borderRadius: 12, maxWidth: "70%", marginBottom: 6, position: "relative" }}>
                  <div>{m.text}</div>
                  <div style={{ fontSize: 10, color: "#555", position: "absolute", bottom: 2, right: 8 }}>{formatTime(m.time)}</div>
                </div>
              );
            })}
          </div>
          <form onSubmit={sendMessage} style={{ display: "flex", borderTop: "1px solid #ddd", padding: 8, gap: 8 }}>
            <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="Type a message..." style={{ flex: 1, padding: 8 }} />
            <button type="submit">Send</button>
          </form>
        </div>
      </div>
...
function VideoTile({ streamRef, label, videoEnabled }) {
  return (
    <div style={{ background: "#000", borderRadius: 12, overflow: "hidden", position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
      {videoEnabled ? (
        <video ref={streamRef} autoPlay playsInline muted style={{ width: "100%", height: "100%" }} />
      ) : (
        <div style={{ width: 80, height: 80, borderRadius: "50%", background: "#4caf50", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32 }}>
          {label[0].toUpperCase()}
        </div>
      )}
      <span style={{ position: "absolute", bottom: 8, left: 8, color: "#fff" }}>{label}</span>
    </div>
  );
}
...
function RemoteTile({ stream, label }) {
  ...
  return (
    <div style={{ background: "#000", borderRadius: 12, overflow: "hidden", position: "relative" }}>
      <video ref={vid} autoPlay playsInline style={{ width: "100%", height: "100%" }} />
      <span style={{ position: "absolute", bottom: 8, left: 8, color: "#fff" }}>{label}</span>
    </div>
  );
}
